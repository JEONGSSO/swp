...
//			return new ResponseEntity<String>(T body, status)		// ResponseEntity에 T는 어떤타입이든 와도 된다. object임	
	...
	@RequestMapping(value = "/{uid}/{mno}", method = RequestMethod.GET) // POST 등록
	public ResponseEntity<MessageVo> readMessage(@PathVariable String uid, @PathVariable int mno)	// 스트링을 리턴할거야	json하려면 obj가와야한다.
	{ 																							//json은 받아와서 vo를 만들거야, 잭슨이 받아온다(?)
																									//@RequestBody는 json의 바디에  클라이언트에서 준 값을 message에 담는다.
																									//ResponseEntity 주고받는 봉투를 구현한것,  데이터가 예외적 상황에 상태 표현해주는 것 값은 메시지vo
		try
		{
			MessageVo msgvo = service.readMessage(uid, mno);	//위에서 담은 reply를 서비스에 레지스터 실행
			return new ResponseEntity<>(msgvo, HttpStatus.OK); 		//ok == 200;
//			return new ResponseEntity<String>(T body, status)		// ResponseEntity에 T는 어떤타입이든 와도 된다. object임	
		...
		{
			e.printStackTrace();	//예외를 출력해주고
			return new ResponseEntity<>(HttpStatus.BAD_REQUEST);	//BAD_REQUEST == 400을 반환한다.
		}
